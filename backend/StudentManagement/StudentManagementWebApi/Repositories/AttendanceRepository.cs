...
public class AttendanceRepository : IAttendanceRepository
{
    ...
    public AttendanceRepository(IConfiguration configuration)
    {
        ...
        _attendanceMapper = new AttendanceMapper(_studentMapper);
    }

    public List<AttendanceDto> GetAllAttendances(string groupName, string academicYear, string subjectName,
        string sessionType)
    {
        var query = "SELECT * " +
                    ...
                    "' AND S.SubjectName = '" + subjectName + "' AND ST2.TypeName = '" + sessionType + "'";
    ...
    public void AddAttendance(AttendanceCreationDto attendanceDto)
    {
        try
        {
            ...
            var query =
                "INSERT INTO Attendance (StudentID, TeacherID, TypeID, Date, Time, Status, Comments, Auditorium, SubjectID, GroupID) " +
                "VALUES (" + attendanceEntity.StudentID + ", " + attendanceEntity.TeacherID + ", " +
                attendanceEntity.TypeID + ", '" + attendanceEntity.Date + "', '" + attendanceEntity.Time + "', '" +
                attendanceEntity.Status + "', '" + attendanceEntity.Comments + "', '" + attendanceEntity.Auditorium +
                "', " + attendanceEntity.SubjectID + ", " + attendanceEntity.GroupID + ")";
